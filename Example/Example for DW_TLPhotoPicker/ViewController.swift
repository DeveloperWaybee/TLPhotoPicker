...
class ViewController: UIViewController,DW_TLPhotosPickerViewControllerDelegate {
    ...
    @IBAction func pickerWithCustomLayout() {
        let viewController = DW_TLPhotosPickerViewController()
        viewController.delegate = self
        viewController.didExceedMaximumNumberOfSelection = { [weak self] (picker) in
            self?.showExceededMaximumAlert(vc: picker)
        }
...
extension ViewController: DW_TLPhotosPickerLogDelegate {
